...
If we have separate wrap counts for two zones: the first half and second half of the 32 counter, then we have a long time to update the wrap counter for the zone that is not in use.
...
## lockless periodic update

Using the lockless function requires that the zones counts are updated at least twice before the 32 counter wraps.  perhaps every operating system tick:
...
The above functions use the following common for reading the cycle count or disabling/restoring interrupts:
